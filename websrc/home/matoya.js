...
// (not-so) Private helpers

export function mty_mem() {
	return MTY.module.instance.exports.memory.buffer;
}

export function mty_mem_view() {
	return new DataView(mty_mem());
}

export function mty_char_to_js(buf) {
	let str = '';
...
export function mty_b64_to_buf(str) {
	return Uint8Array.from(atob(str), c => c.charCodeAt(0))
}

export function mty_buf_to_b64(buf) {
	let str = '';
...
const MTY_WEB_API = {
	...
	web_attach_events: function (app, mouse_motion, mouse_button, mouse_wheel, keyboard, focus, drop, resize) {
		console.error("Ataching Web events");
		...
		function mouseMoveEvent(ev) {
			...
			MTY_CFunc(mouse_motion)(MTY.app, 0, MTY.relative, x, y);
		}

		MTY.gl.canvas.removeEventListener('mousemove', mouseMoveEvent);
		MTY.gl.canvas.addEventListener('mousemove', mouseMoveEvent);

		function pointerLockEventChange(ev) {
			// Left relative via the ESC key, which swallows a natural ESC keypress
			if (!document.pointerLockElement && MTY.synthesizeEsc) {
				MTY_CFunc(keyboard)(MTY.app, true, MTY.keys['Escape'], 0, 0);
				MTY_CFunc(keyboard)(MTY.app, false, MTY.keys['Escape'], 0, 0);
			}

			MTY.synthesizeEsc = true;
		}

		document.removeEventListener('pointerlockchange', pointerLockEventChange);
		document.addEventListener('pointerlockchange', pointerLockEventChange);

		// Only attach events once
		if (MTY.attached_events) return;
		MTY.attached_events = true;
		...
		window.addEventListener('mousedown', (ev) => {
			...
			ev.preventDefault();
			MTY_CFunc(mouse_button)(MTY.app, 0, true, ev.button, mty_scaled(ev.clientX), mty_scaled(ev.clientY));
		});

		window.addEventListener('mouseup', (ev) => {
			ev.preventDefault();
			MTY_CFunc(mouse_button)(MTY.app, 0, false, ev.button, mty_scaled(ev.clientX), mty_scaled(ev.clientY));
		});
		...
		var touch_started = function (ev) {
			...
			for (var i = 0; i < touches.length; i++) {
				...
				mty_correct_relative();
				MTY_CFunc(mouse_button)(MTY.app, touch.identifier, true, 0, mty_scaled(touch.clientX), mty_scaled(touch.clientY));
			}
		};
		...
		var touch_moved = function (ev) {
			...
			for (var i = 0; i < touches.length; i++) {
				...
				if (touchIndex != -1) {
					...
					MTY_CFunc(mouse_motion)(MTY.app, touch.identifier, MTY.relative, touch.clientX, touch.clientY);
				}
			}
		};
		...
		var touch_ended = function (ev) {
			...
			for (var i = 0; i < touches.length; i++) {
				...
				if (touchIndex != -1) {
					...
					MTY_CFunc(mouse_button)(MTY.app, touch.identifier, false, 0, mty_scaled(touch.clientX), mty_scaled(touch.clientY));
				}
			}
		};
		...
		MTY.gl.canvas.addEventListener('wheel', (ev) => {
			...
			MTY_CFunc(mouse_wheel)(MTY.app, x, y);
		}, {passive: true});
		...
		window.addEventListener('blur', (ev) => {
			MTY_CFunc(focus)(MTY.app, false);
		});

		window.addEventListener('focus', (ev) => {
			MTY_CFunc(focus)(MTY.app, true);
		});

		window.addEventListener('resize', (ev) => {
			MTY_CFunc(resize)(MTY.app);
		});
				...
				if (ev.dataTransfer.items[x].kind == 'file') {
					...
					reader.addEventListener('loadend', (fev) => {
						if (reader.readyState == 2) {
							...
							MTY_Memcpy(cmem, buf);
							MTY_CFunc(drop)(MTY.app, MTY_StrToC(file.name, MTY.cbuf, 1024), cmem, buf.length);
							MTY_Free(cmem);
						}
					});
...
export function MTY_Stop() {
	// Created for libRR
	console.error("MTY_Stop", MTY);
	console.log("removing old file buffer:")
...
export async function MTY_Start(bin, userEnv, endFunc, glver) {
	MTY.arg0 = bin;
